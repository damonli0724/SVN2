...
public class CustomExcelHelper {
	 ...
	 */
	public static void registerNewType(@SuppressWarnings("rawtypes") Class<? extends ExcelType> type) throws ExcelParseException {
	 ...
	 */
	public static CustomExcelHelper readExcel(String excelFilename) throws InvalidFormatException, IOException {
	 ...
	 */
	public static CustomExcelHelper readExcel(String excelFilename, int sheetIndex) throws InvalidFormatException, IOException {
	 ...
	 * 2013-11-28 下午3:40:05
	 */
	public static CustomExcelHelper readExcel(File file) throws InvalidFormatException, IOException {
	 ...
	 * 2013-11-28 下午3:40:05
	 */
	public static CustomExcelHelper readExcel(File file, int sheetIndex) throws InvalidFormatException, IOException {
		// 读取Excel工作薄
	 ...
	 * @throws IOException
	 */
	public static CustomExcelHelper readExcel(InputStream ins) throws InvalidFormatException, IOException {
	 ...
	 * @throws IOException
	 */
	public static CustomExcelHelper readExcel(InputStream ins, int sheetIndex) throws InvalidFormatException, IOException {
		...
		try {
			...
			for (String[] data : datas) {
				...
				for (ExcelEntityField eef : eefs) {
					...
					// continue;
					// }
					// todo:alvin:规则校验 start
					if (eef.isRequired() && StringUtil.isEmpty(data[eef.getIndex()])) {// 必须
						throw new ExcelParseException("Excel数据的第" + lineNo + "行第" + (eef.getIndex() + 1) + "列字段" + eef.getColumnName() + "为空!");
					}
					// todo:alvin:规则校验 end
					// 实体数据填充
					Method method = obj.getClass().getDeclaredMethod("set" + _toCapitalizeCamelCase(eef.getField().getName()), eef.getField().getType());
					try {
						...
						if (eef.isRequired()) {
							...
							throw new ExcelContentInvalidException("Excel数据的第" + lineNo + "行第" + (eef.getIndex() + 1) + "列字段错误:" + e.getMessage());
						}
						continue;
	...
	private <T> List<ExcelEntityField> _getEntityFields(Class<T> classType) throws ExcelParseException {
		List<ExcelEntityField> eefs = new ArrayList<CustomExcelHelper.ExcelEntityField>();
		// 遍历所有字段
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private Object _getFieldValue(String value, ExcelEntityField eef) throws ExcelParseException, InstantiationException, IllegalAccessException,
			ExcelContentInvalidException, ExcelRegexpValidFailedException {
		// 进行规则校验
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private Object _getFieldValue(String value, Field field, String regexp) throws ExcelParseException, ExcelContentInvalidException,
	...
	private void _parseExcelData(Sheet sheet, int rowStart, int rowEnd) {
		...
		for (int rowIndex = rowStart; rowIndex <= rowEnd; rowIndex++) {
			...
			for (int columnIndex = 0; columnIndex < lastColumnIndex; columnIndex++) {
				...
				// 如果该行的第一列为空,则结束这个循环
				if (StringUtil.isEmpty(value) && columnIndex == 0) {
					break;
				}
		...
		switch (cell.getCellType()) {
			...
			case Cell.CELL_TYPE_NUMERIC:
				if (DateUtil.isCellDateFormatted(cell)) {
					...
					if (cell.getNumericCellValue() - (int) cell.getNumericCellValue() == 0) {// 如果小数部分
						int intValue = (int) cell.getNumericCellValue();															// 等于0
	 ...
	 * @return
	 */
	private static CustomExcelHelper _readExcel(Workbook wb, int sheetIndex) {
		CustomExcelHelper eh = new CustomExcelHelper();
