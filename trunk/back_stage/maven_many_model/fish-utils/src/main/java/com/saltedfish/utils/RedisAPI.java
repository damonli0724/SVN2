...
public class RedisAPI {
	...
	public static JedisPool getPool() {
		if (pool == null) {
			...
			ResourceBundle bundle = ResourceBundle.getBundle("system-config");
			if (bundle == null) 
				throw new IllegalArgumentException( "[system-config] is not found!");
			...
			pool = new JedisPool(config, bundle.getString("redis.host"), Integer.valueOf(bundle.getString("redis.port")), 1000000, String.valueOf(bundle.getString("redis.pass")));
			pool.getResource().set("redis:watch:seckill:account", "100"); 
		} 
		return pool;
	}
