...
$(function(){
	...
	/*搜索框*/
	$.Huifocusblur('.sesaltedfishTxt');
	$.Huihover('.ac_results li');
	$(".ac_results li").click(function (event){
		$(".sesaltedfishTxt").addClass("focus").val($(this).find("p").text());
		$(".ac_results").hide();
		//$(".form-sesaltedfish").submit();/*提交表单*/
		b_onclick();/*临时测试*/
		return false;
	});
	$(".sesaltedfishTxt").focus(function(){$(".ac_results").show();return false;});
	$(".ac_results").blur(function(){$(this).hide();});
	$("body").click(function(){$(".ac_results").hide();});
	$(".sesaltedfishTxt").click(function(){$(".ac_results").show();return false;});
	function BindEnter(obj){
    	var sesaltedfishBtn = $("#sesaltedfishBtn");
    	if(obj.keyCode == 13){sesaltedfishBtn.click();obj.returnValue = false;}
	}
